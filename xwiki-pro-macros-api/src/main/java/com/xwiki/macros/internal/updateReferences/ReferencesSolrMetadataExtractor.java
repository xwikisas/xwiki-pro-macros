...
import static com.xwiki.macros.internal.updateReferences.listeners.PageRenameMacroParametersUpdateListener.MACROS_TO_SEARCH;
...
public class ReferencesSolrMetadataExtractor implements SolrEntityMetadataExtractor<XWikiDocument>
{
    ...
    public boolean extract(XWikiDocument document, SolrInputDocument solrDocument)
    {
        ...
        OrBlockMatcher orBlockMatcher =
            new OrBlockMatcher(MACROS_TO_SEARCH.keySet()
                .stream()
                .map(MacroBlockMatcher::new)
                .collect(Collectors.toList()));

        List<Block> macroBlocks = xdom.getBlocks(orBlockMatcher, Block.Axes.DESCENDANT);
        ...
        if (macroBlocks != null && !macroBlocks.isEmpty() && !updateMacroReference(document, xdom, macroBlocks)) {
            ...
            for (Block macroBlock : macroBlocks) {
                if (macroBlock instanceof MacroBlock) {
                    List<String> parameters = MACROS_TO_SEARCH.get(((MacroBlock) macroBlock).getId());
    ...
    private boolean updateMacroReference(XWikiDocument document, XDOM xdom, List<Block> macroBlocks)
    {
        try {
            ...
            for (Block macroBlock : macroBlocks) {
                if (macroBlock instanceof MacroBlock) {
                    String macroName = ((MacroBlock) macroBlock).getId();
                    List<String> parameters = MACROS_TO_SEARCH.get(macroName);
                    for (String parameter : parameters) {
                        ...
                        if (!documentExists) {
                            ...
                            if (!isValid) {
                                ...
                                logger.debug(
                                    "The reference [{}] was updated to [{}] to respect the current name strategy. "
